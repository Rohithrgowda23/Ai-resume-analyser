...
function Forgotpassword() {
    ...
    const verifyemail = () => {
        ...
        setisloading(true)
        fetch(`${backendURL}/send-reset-otp`, {
            method: "POST",
            headers: {
    ...
    const verifyprocess = () => {
        ...
        setisloading(true)
        fetch(`${backendURL}/verify-reset-otp`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
        ...
        <div className={Styles.container}>
            ...
            <div className={Styles.center}>
                {!isemailpresent && !isemailverified ?
                    <div className={`${Styles.mailcontainer} card reveal`}>
                        <span className="badge">Reset password</span>
                        <h1>Email Address</h1>
                        <p className={Styles.hint}>We'll send a 6-digit code to verify it's you.</p>
                        ...
                        <button className="btn btn-primary btn-block" onClick={verifyemail} disabled={isloading}>{isloading ? "Loading..." : "Submit"}</button>
                    </div> : null}

                {isemailpresent && !isemailverified ?
                    <div className={`${Styles.verifycontainer} card reveal`}>
                        <span className="badge">Verify code</span>
                        <h1>Verify Email</h1>
                        ...
                        <button onClick={verifyprocess} className={`btn btn-primary btn-block ${Styles.verbtn}`} disabled={isloading}  >{isloading ? "Verifying..." : "Verify"}</button>
                    </div> : null}
                ...
                {isemailpresent && isemailverified ?
                    <div className={`${Styles.mailcontainer} card reveal`}>
                        ...
                        <button className="btn btn-primary btn-block" onClick={resetpasswordsent} disabled={isloading}>{isloading ? "Changing..." : "Change"}</button>
                    </div> : null}
            </div>
        </div>
    )
